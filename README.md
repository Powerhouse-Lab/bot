...
The workflow also runs on pushes to `main` when app source, config, or workflow files change. It intentionally uses `npm install` without the setup-node npm cache so it can run before a `package-lock.json` exists. The APK is signed with a CI-generated throwaway keystore, which is useful for sideloading and testing. Use a real upload/release keystore before distributing through Google Play or long-term release channels.
